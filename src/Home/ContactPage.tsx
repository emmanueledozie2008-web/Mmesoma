...
import { 
  FaEnvelope, FaUser, FaPhone, FaCalendarAlt, FaFileAlt, 
...
const ContactPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validation
    if (!formData.name || !formData.email || !formData.scamType || !formData.description) {
    ...
    try {
      const templateParams = {
        ...
        attachment: formData.attachment ? formData.attachment.name : t('noFile'),
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="bg-[#0B3B60] text-white py-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">{t('contactTitle')}</h1>
          <p className="text-gray-200 max-w-2xl mx-auto">{t('contactSubtitle')}</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-400 mt-1">{t('attachmentHint')}</p>
            </div>
            ...
            <div className="pt-4">
              ...
              >
                {isSubmitting ? t('sending') : <><FaPaperPlane /> {t('submitButton')}</>}
              </button>
            </div>

            {/* Status Messages */}
            {submitStatus === 'success' && (
              <div className="mt-4 p-3 bg-green-100 border-l-4 border-green-500 text-green-700 rounded flex items-center gap-2">
                <FaCheckCircle /> {t('successMessage')}
              </div>
            )}
            {submitStatus === 'error' && (
