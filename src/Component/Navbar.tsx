...
import { FaBars, FaTimes, FaShieldAlt, FaGripLines } from 'react-icons/fa';
...
const Navbar: React.FC = () => {
  ...
  // Drag handlers (unchanged, works for draggable widget)
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      newX = Math.min(window.innerWidth - (dragRef.current?.offsetWidth || 120), Math.max(0, newX));
      newY = Math.min(window.innerHeight - (dragRef.current?.offsetHeight || 50), Math.max(0, newY));
  ...
  // Helper: handle external link click (closes sidebar, then opens link)
  const handleExternalLink = (url: string) => {
    setIsSidebarOpen(false);          // close mobile menu first
    window.open(url, '_blank', 'noopener,noreferrer');
  };

  // Helper: internal link click (closes sidebar, let React Router handle)
  ...
  const renderDesktopLink = (link: NavLink) => {
    const classes = "px-3 py-2 rounded-md text-sm font-medium text-gray-200 hover:text-white hover:bg-[#B22234]/20 transition-all";
    if (link.external) {
      return (
        <a key={link.name} href={link.href} target="_blank" rel="noopener noreferrer" className={classes}>
          {link.name}
        </a>
      );
    }
  ...
  // Mobile link renderer (closes sidebar properly)
  const renderMobileLink = (link: NavLink) => {
    const classes = "px-3 py-2 rounded-md text-gray-700 hover:text-[#B22234] hover:bg-[#B22234]/10 transition-colors block w-full text-left";
    if (link.external) {
      return (
        <button
          key={link.name}
    ...
    return (
      <Link key={link.name} to={link.href} onClick={handleInternalLink} className={classes}>
        {link.name}
      </Link>
    );
  };
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-center h-16 md:h-20">
            ...
            <Link to="/" className="flex items-center space-x-2">
              ...
              <div className="flex flex-col">
                <span className="text-white font-bold text-sm md:text-lg tracking-wider">FEDERAL BUREAU</span>
                <span className="text-[#FFD700] text-[10px] md:text-xs tracking-widest -mt-1">OF INVESTIGATION</span>
              </div>
            </Link>
      ...
      {/* Draggable Widget (optional) – uncomment if needed */}
      {/* 
      <div
        ref={dragRef}
      ...
      >
        ...
        <span className="text-sm font-medium tracking-wide">🔹 Main FBI Website</span>
      </div>
      */}
    </>
  );
